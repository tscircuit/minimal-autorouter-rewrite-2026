...
bun scripts/benchmark/run.ts --module .benchmark/baseline/node_modules/@tscircuit/capacity-autorouter/dist/index.js --concurrency 2 --output .benchmark/baseline-results.json
bun scripts/benchmark/run.ts --module lib/index.ts --concurrency 2 --output .benchmark/rewrite-results.json
...
The comparison command requires matching sample hashes, DRC oracle, Bun version, CPU, platform, and concurrency. It rejects any per-sample loss of completion or relaxed DRC, aggregate regressions, more timeouts, or higher average via count. Its default p50/p95 allowance is 1.1 times the baseline to accommodate timing noise; pass a third argument of `1` for a strict time gate. Length is reported separately. A passing aggregate alone must not hide a newly failing sample.
...
For either remote pass, the harness requires exposed remote metrics and rejects any local transport fallback. A hot pass must have every remote request satisfied by a cache hit, no batch misses, no single solve requests, and no solver results. Local routing and transport contract tests alone do not establish remote cold/hot benchmark parity. Reports without remote cache settings make no claim about remote cache performance.
