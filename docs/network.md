...
The public service at `hd-cache2.tscircuit.com` advertised version `0.0.856` when inspected; it does not accept this package's `0.1.0` implementation version. Its published schema also limits node sizes and obstacle types. No external routing requests were used in development or benchmark claims. The exact board extension requires this repository's service or a service that explicitly implements and advertises the same contract. See `network-completion-proposal.md` for the pinned primary-source audit.
